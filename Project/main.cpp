...
int main() {
  ...
  while (true) {
    ...
    string dstIP = inet_ntoa(ip_hdr->ip_dst);
    cout << "Outer Source IP: " << srcIP << '\n';
    cout << "Outer Destination IP: " << dstIP << '\n';

    if (+ip_hdr->ip_p == 17) {
      cout << "Next Layer Protocol: UDP\n\n";

      udphdr *udp_hdr = (struct udphdr *) pkt_cursor;
      pkt_cursor += sizeof(struct udphdr);
      uint16_t sport = ntohs(udp_hdr->source);
      uint16_t dport = ntohs(udp_hdr->dest);
      cout << "Source Port: " << dec << sport << '\n';
      cout << "Destination Port: " << dec << dport << '\n';

      pkt_cursor += 2; // Note: Ignore GRE header flag and version (assuming it only contains key)
      cout << "Protocol: " << setw(2) << setfill('0') << hex << +pkt_cursor[0] << +pkt_cursor[1] << '\n';
      pkt_cursor += 2;
      uint32_t key = ntohl(*(uint32_t *) pkt_cursor);
      cout << "Key: " << dec << key << '\n';
      pkt_cursor += 4;
      ...
      system(("ip fou add port " + to_string(dport) + " ipproto 47").c_str());
      string dev_name = "GRETAP" + to_string(packet_num);
      system(("ip link add " + dev_name + " type gretap remote " +=
              srcIP + " local " += dstIP + " key " + to_string(key) + " encap fou encap-sport " + to_string(dport)
                  + " encap-dport " + to_string(sport)).c_str());
      system(("ip link set " + dev_name + " up").c_str());
      system(("ip link set " + dev_name + " master br1").c_str());

      filter_exp += "&& src port not " + to_string(sport);
      if (pcap_compile(handle, &filter, filter_exp.c_str(), 0, PCAP_NETMASK_UNKNOWN) == PCAP_ERROR) {
        cout << "pcap_compile() " << pcap_geterr(handle) << '\n';
        exit(EXIT_FAILURE);
      }
