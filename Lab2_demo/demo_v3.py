...
def config(hosts, switches, routers, DHCPServer):
    ...
    # DHCP
    DHCPServer.cmd('ifconfig DHCPServer-eth0 192.168.3.4/26')
    # h2
    hosts['h2'].cmd('ifconfig h2-eth0 192.168.3.3/26')
    # h3
    hosts['h3'].cmd('ifconfig h3-eth0 192.168.3.66/26')
    # h4
    hosts['h4'].cmd('ifconfig h4-eth0 192.168.4.1/24')
    # h5
    hosts['h5'].cmd('ifconfig h5-eth0 192.168.4.2/24')
    
    hosts['h2'].cmd('route add default gw 192.168.3.62')
    
    hosts['h3'].cmd('route add default gw 192.168.3.126')

    hosts['h4'].cmd('route add default gw 192.168.4.254')

    hosts['h5'].cmd('route add default gw 192.168.4.254')

    #Routers interface IP configuration
    # R1
    routers['r1'].cmd('ifconfig r1-eth0 10.0.1.2/24')
    routers['r1'].cmd('ifconfig r1-eth1 192.168.3.62/26')
    routers['r1'].cmd('ifconfig r1-eth2 192.168.3.126/26')
    ...
    routers['r4'].cmd('ifconfig r4-eth1 192.168.4.254/24')

    # Router routing table configuration
    routers['r1'].cmd('route add -net 192.168.4.0/24 gw 10.0.1.1')
    routers['r2'].cmd('route add -net 192.168.3.0/25 gw 10.0.1.2')
    routers['r2'].cmd('route add -net 192.168.4.0/24 gw 10.0.0.2')
    routers['r3'].cmd('route add -net 192.168.3.0/25 gw 10.0.0.1')
    routers['r3'].cmd('route add -net 192.168.4.0/24 gw 10.0.2.3')
    routers['r4'].cmd('route add -net 192.168.3.0/24 gw 10.0.2.1')

def check(hosts):
    ips = {'192.168.3.3', '192.168.3.66', '192.168.4.1', '192.168.4.2'}
    flag = 0
